...
object hlist {
 ...
 trait Grouper[L <: HList, N <: Nat, Step <: Nat] extends DepFn1[L] with Serializable {
   ...
   * @author Ivan Aristov
   */
  @implicitNotFound("Implicit not found: shapeless.Ops.DropAt[${L}, ${N}]. You requested to drop an element at the position ${N}, but the HList ${L} is too short.")
  trait DropAt[L <: HList, N <: Nat] extends DepFn1[L] with Serializable { type Out <: HList }

  object DropAt {
    def apply[L <: HList, N <: Nat](implicit drop: DropAt[L, N]): Aux[L, N, drop.Out] = drop

    type Aux[L <: HList, N <: Nat, Out0 <: HList] = DropAt[L, N] { type Out = Out0 }

    implicit def hlistDrop1[H, L <: HList]: Aux[H :: L, _0, L] =
      new DropAt[H :: L, _0] {
        type Out = L
        def apply(l : H :: L): Out = l.tail
      }

    implicit def hlistDrop2[H, T <: HList, N <: Nat, Out0 <: HList](
      implicit dt: DropAt[T, N]
    ): Aux[H :: T, Succ[N], H :: dt.Out] =
      new DropAt[H :: T, Succ[N]] {
        type Out = H :: dt.Out
