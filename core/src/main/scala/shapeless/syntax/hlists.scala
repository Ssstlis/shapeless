...
final class HListOps[L <: HList](l : L) extends Serializable {
  ...
  /**
   * Returns all but the ''n'' element of this `HList`. Available only if there is evidence that this `HList`
   * has at least ''n'' elements.
   */
  def dropAt(n : Nat)(implicit drop : DropAt[L, n.N]) : drop.Out = drop(l)

  /**
   * Returns all but the ''n'' element of this `HList`. An explicit type argument must be provided. Available
   * only if there is evidence that this `HList` has at least ''n'' elements.
   */
  def dropAt[N <: Nat](implicit drop : DropAt[L, N]) : drop.Out = drop(l)
