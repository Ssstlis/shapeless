...
final class TupleOps[T](t: T) extends Serializable {
  ...
  /**
   * Returns all but remove the ''n'' element of this tuple. An explicit type argument must be provided. Available
   * only if there is evidence that this tuple has at least ''n'' elements.
   */
  def dropAt[N <: Nat](implicit drop: DropAt[T, N]): drop.Out = drop(t)

  /**
   * Returns all but remove the ''n'' element of this tuple. Available only if there is evidence that this tuple
   * has at least ''n'' elements.
   */
  def dropAt(n: Nat)(implicit drop: DropAt[T, n.N]): drop.Out = drop(t)
